#' Proximal Direct Standardization (ProxDS)
#'
#' Performs a locally weighted direct standardization using Gaussian-weighted proximity
#' matrices computed from calibration (child) spectra. Each prediction sample is
#' corrected using a locally fitted transformation derived from its nearest calibration
#' neighborhood.
#'
#' @param child_trans  Matrix or data frame of calibration (child) spectra.
#' @param parent_trans Matrix or data frame of reference (parent) spectra.
#' @param child_mod    Matrix or data frame of spectra to be corrected (child model space).
#' @param lambda       Regularization parameter (default = 1e-3).
#'
#' @return A matrix of corrected spectra (same dimensions as \code{child_mod}).
#'         The result is also assigned to the global environment as \code{child.mod_cor}.
#'
#' @examples
#' \dontrun{
#' corrected <- ProxDS(child_trans, parent_trans, child_mod, lambda = 1e-3)
#' }
#' @export
#' @import doParallel
