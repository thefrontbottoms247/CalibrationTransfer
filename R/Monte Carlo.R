#' @title Monte Carlo PLS Transfer Evaluation
#' @description
#' Performs repeated random sub-sampling validation comparing parent vs. corrected-child
#' PCA/PLS models. Reports RMSE distributions, percent differences, and summary statistics.
#'
...
Monte_Carlo <- function(parent,
  ...
  # --- Input prep ---
  if (is.matrix(parent)) parent <- as.data.frame(parent)
  if (is.matrix(child_cor)) child_cor <- as.data.frame(child_cor)
  parent <- as.data.frame(lapply(parent, as.numeric))
  child_cor <- as.data.frame(lapply(child_cor, as.numeric))
  class <- as.numeric(class)

  if (nrow(parent) != length(class) || nrow(child_cor) != length(class))
  ...
  # Percent Δ (child vs parent)
  df.monte$delta_pct <<- 100 * (rmse_child_corr - rmse_parent_self) / rmse_parent_self
  ...
  cat("\n=== Percent difference (Child vs Parent) ===\n")
  delta_pct <- df.monte$delta_pct
  cat("Mean Δ%:", round(mean(delta_pct), 3),
      "| Median Δ%:", round(median(delta_pct), 3),
      "| SD Δ%:", round(sd(delta_pct), 3),
      "| Range:", paste0(round(range(delta_pct), 3), collapse = " - "), "\n")

  thr <- c(0.25, 0.5, 1, 2, 5)
  cat("\n=== Share within ±x% ===\n")
  for (t in thr) {
    pct <- mean(abs(delta_pct) <= t) * 100
    cat(sprintf("±%-5.2f%% : %5.1f%% of iterations\n", t, pct))
  }
  cat("\nImproved (Child < Parent):", sum(delta_pct < 0),
      "| Worse (Child > Parent):", sum(delta_pct > 0), "\n")
  ...
  print(p)
  ggsave("Monte_Carlo_RMSE.png", p, width = 7.9, height = 7.68, dpi = 300)
}
